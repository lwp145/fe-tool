...
<template>
  <div class="tool_box">
	...
	<!-- 主内容 start -->
      ...
      <div class="api_p">
        <b>代码换算1.0-API   陈海崇 开发于2015-05  QQ5654565</b> 
        <p><span>1.倍数：</span>默认有3单选的倍数，分别是1.5倍与2倍，如非这个倍数的，则勾选上‘其他’的单选输入相应的倍数即可，支持小数点但是输入的数字必须大于1，如果小于1的则不做倍数计算.</p>
        <p><span>倍数计算结果的取整方式为：</span>因为计算结果如果存在奇数，有些安卓设备会出现兼容性问题，比如字体为奇数时在安卓设备会出现毛边现象，所以这里的计算结果都会取整位偶数，当计算结果非偶数时，则把结果除以2.余数大于或等于1则向上取整为偶数，如果余数小于1则向下取值为偶数.</p>
        <p><span>2.单选-过滤非像素设置的CSS属性：</span>默认：勾选</p>
        <p>在换算代码过程中是否对非像素的CSS属性进行筛选，如：a{ display: block; 　font-size: 18px;},如果勾选后的结果为a{ font-size: 18px;},跟像素无关的CSS属性则被过滤掉了</p>
        <p><span>3.删除过滤后产生空白的CLASS：</span>默认：勾选</p>
        ...
        <p><span>4过滤text-indent为负：</span>默认：勾选</p> 
        ...
        <p><span>5.过滤border边框</span>默认：不勾选</p>
        ...
        <p><span>6.自定义过滤属性" : " 隔开：</span>默认：不勾选</p>
...
.textarea textarea{font-size: 14px !important;}
.column { padding-left: 80px; position: relative; font-size: 0; min-height: 30px; margin: 5px 0; }
.column .column_a1 { position: absolute; left: 0; top: 50%; margin-top: -15px; display: block; width: 60px; height: 30px; line-height: 30px; font-size: 16px; background-color: #ff6a00; color: #fff; text-align: center; }
/*尺寸*/
.column_box1 { font-size: 0; padding: 6px 6px 6px 0px; margin-bottom: 6px; }
.column_num { display: inline-block; font-size: 0; vertical-align: top; height: 30px; vertical-align: top; }
.column_num label { display: inline-block; white-space: nowrap; height: 30px; margin: 0 6px; vertical-align: top; }
.column_num label span { display: inline-block; vertical-align: top; height: 30px; line-height: 30px; margin-right: 4px; font-size: 16px; color: #000; }
.column_num label input[type='radio'] { display: inline-block; vertical-align: top; border: 1px solid #000; margin-top: 8px; }
.column_num b { display: inline-block; vertical-align: top; height: 30px; line-height: 30px; font-size: 16px; color: #000; font-weight: normal; }
.column_num input[type='text'] { display: inline-block; vertical-align: top; width: 60px; height: 28px; line-height: 28px; font-size: 16px; color: #000; overflow: hidden; text-indent: 10px; border: 1px solid #8f8f8f; }
/*单位*/
.column_unit { display: inline-block; font-size: 0; vertical-align: top; height: 30px; vertical-align: top; border-right: 1px solid #808080; margin-right: 10px; }
.column_unit label { display: inline-block; white-space: nowrap; height: 30px; margin: 0 6px; vertical-align: top; }
.column_unit label span { display: inline-block; vertical-align: top; height: 30px; line-height: 30px; margin-right: 10px; font-size: 16px; color: #000; }
.column_unit label input[type='radio'] { display: inline-block; vertical-align: top; border: 1px solid #000; margin-top: 8px; }
.column_unit b { display: inline-block; vertical-align: top; height: 30px; line-height: 30px; font-size: 16px; color: #000; font-weight: normal; }
/*选择*/
.column_box2 {}
.column_checkbox { display: inline-block; white-space: nowrap; height: 30px; margin-right: 30px; vertical-align: top; }
.column_checkbox span { display: inline-block; vertical-align: top; height: 30px; line-height: 30px; margin-right: 10px; font-size: 16px; color: #000; }
.column_checkbox input[type='checkbox'] { display: inline-block; vertical-align: top; border: 1px solid #000; margin-top: 8px; }
.column_checkbox input[type='text'] { display: inline-block; vertical-align: top; width: 310px; height: 28px; line-height: 28px; font-size: 16px; color: #000; overflow: hidden; text-indent: 6px; border: 1px solid #8f8f8f;margin-left:10px; }
/*API*/
.api_p {padding-bottom:50px;}
.api_p b { display: block; font-size: 20px; line-height: 35px; }
.api_p p { display: block; font-size: 18px; line-height: 32px; white-space: normal; }
.api_p span { font-weight: bold;}
</style>
...
export default {
  ...
  methods:{
      ...
      if (r_checkbox4) {
          agoVal = agoVal.replace(/\s*border\s*:[^;]*;/g, '')
      ...
      if (r_checkbox5) { 
          ...
          if ($.trim(rb5_val.replace(/:/g, '')) != '') {
              ...
              for (var i = 0; i < rb5_length; i++) {
                  ...
                  if (rb5_arrayVal != '') { 
                      var rb5_regexp = new RegExp('\\s*' + rb5_arrayVal + '\\s*:[^;]*;');  
