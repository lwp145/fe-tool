...
<template>
  <div class="tool_box">
	  ...
	  <!-- 主内容 start -->
      <div id="container" >
          <a id="selectBtn" class="myButton" href="javascript:void(0);" onclick="document.getElementById('input_file').click();">选择图片</a>
          <!-- <a id="saveBtn" href="javascript:void(0);" onclick="saveImage();">保存</a> -->
          <input type="file" id="input_file" size="10" style="display:none;" @change="selectImage($event['path'][0])" />
          
          <div id="wrapper">
            
            <!-- 原图 -->
            <div class="artwork">
              <p class="pic_info_p">width：<strong>{{picWidth}}</strong>px　height：<strong>{{picHeight}}</strong>px　size：<strong>{{picSize}}</strong>kb</p>
              <canvas id="cropper"></canvas>
                
              <p class="p_btn" style="display:none;">
                <a class="_btn" id="rotateLeftBtn" href="javascript:void(0);" @click="rotateImage($event);">向左旋转</a>
                <a class="_btn" id="rotateRightBtn" href="javascript:void(0);" @click="rotateImage($event);">向右旋转</a>
              </p>
            </div>
            ...
            <div id="thumbnail" class="thumbnail">
               <div class="thumbnail_box">
                 <!-- <p>180*180</p> -->
                 <p><a v-bind:href="small_base64" v-bind:download="small_pic_name" class="download">Download</a></p>
                 <canvas  id="smallPic" v-bind:width="small_pic_width" v-bind:height="small_pic_height" class="preview"></canvas>
               </div>
            </div>
          </div>
      </div>
      ...
      <!-- <button v-on:click="readFile">aaa</button> -->
	  <!-- 主内容 end -->
  </div>
</template>
    ...
    selectImage:function(fileList){
      ...
      reader.onload = function(e) {
          ...
          new_image.onload = function(){
            ...
            that.cropper = new ImageCropper(img_width, img_height, img_width, img_height);
